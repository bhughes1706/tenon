...
## Bonus Stage — Joint visualization (2026-06-17)

**Problem.** Chunk 9 carves correct joinery, but an assembled mortise & tenon (or any concealed joint) is invisible in the viewport — boards render fully opaque and overlap, so the joint is buried (as in real joinery). This stage adds a **display-only** view layer to reveal it. No geometry, model, op, server, or re-carve changes — three ephemeral view toggles in the store.

**The three tools**
1. **Exploded view** — a `0..1` slider that drifts every board away from the assembly centroid so mating parts separate and the tenon emerges from the mortise. `computeExplodeOffsets` (`web/src/viewport/explode.ts`, pure + unit-tested) reuses `worldAABB` for the centroid/radius, then **axis-snaps** each board's outward direction to its dominant world axis (clean orthographic "blow-apart" for v1's 90° boards). It is **diagrammatic, not kinematic** — a closed frame can't be slid apart by real motion, so we do what CAD does and open all gaps at once. Magnitude = `factor · radius · EXPLODE_GAIN` (0.8). The Viewport adds the offset to each board's `<group>` position; the **gizmo is hidden while exploded** (the board's real transform is elsewhere).
2. **Isolate selected** — an `isolate` slider (0 = off … 1 = fully hidden) fades every board *except* the current selection so a chosen board's joinery reads against ghosted neighbours. **Selection-gated** (`isolate > 0 && selection.length > 0`) so nothing ever fully vanishes with nothing picked; ghost opacity is linear (`1 - isolate`). This **replaced the first-pass "x-ray"** (a uniform fixed-opacity ghost), which had no contrast — both boards faded the same and the buried tenon stayed illegible. The ghost is `transparent + opacity + depthWrite:false` with **a material `key` keyed on the ghosted state** — two non-obvious necessities: (1) toggling `transparent` on an already-rendered material does **not** re-enable blending in R3F, so we remount it (this was also the root of an early "clipping" symptom, where transparency simply never turned on); (2) `depthWrite:false` on the ghost — without it a transparent mesh that writes depth z-fights its own buffer-ordered faces and the coincident solid board, rendering the tenon at the wrong height (this was the depth-sort bug — **now fixed**, see below).
3. **Joint-face highlighting** — `jointFaceMesh(mesh)` (`core/src/eval/jointFaces.ts`, pure + unit-tested) extracts the triangles whose provenance feature carries a `jointId` (mortise walls / tenon cheeks / shoulders — base + edge grooves excluded). `geometryClient.buildBoard` wraps it into a per-board `highlight` BufferGeometry (null for joint-free boards), the store keeps a `jointMeshes` map (built + disposed exactly like `meshes`), and the Viewport renders it as an amber overlay (`resources.jointMat`, polygonOffset so it sits on the coincident base faces, theme token `--vp-joint-hi`). Built eagerly so toggling is instant.

**Reuse note.** The amber `--vp-joint-hi` + `--vp-ghost` tokens, `jointMat`/`ghostMat` materials, and the `jointHighlight`/`ghostMaterial` theme-bridge slots were **provisioned in chunk 7 and unused** — this stage consumes them (extended `jointHighlight` to sync emissive too).

**UI.** Top-bar toggle (Highlight `Highlighter`), a two-row viewport overlay top-right with **explode** + **isolate** sliders (`ViewSlider` in `DesignerShell.tsx`), and ⌘K/registry commands `toggle_isolate` (`Focus`, flips 0↔0.85) / `toggle_joint_highlight` / `explode` (cycles 0→0.5→1). All view state resets on model `load`.

**✅ FIXED (isolate/ghost depth-sort) — 2026-06-17.** Symptom was: a ghosted board interpenetrating a solid (selected) board at a joint rendered the tenon at the wrong height — **bottom half clipped, top flush with the mortise** — with the ghost's outline bleeding through the solid. **The root cause was not inherent** — the earlier "one opaque + one transparent board sharing coincident geometry can't depth-sort in a forward renderer" diagnosis was wrong. The actual cause: the ghost material was `transparent` but left `depthWrite` at its default `true`. A transparent mesh that writes depth (a) z-fights its own faces — Three.js sorts transparent *objects* back-to-front but never sorts triangles *within* a mesh, so raw buffer order decides which survive the depth test — and (b) z-fights the coincident solid board at equal depth. **Fix: one line, `depthWrite={!ghosted}` on the `BoardMesh` material** — the ghost stops writing depth while `depthTest` stays on, so the solid still occludes it correctly. The fear that kept depthWrite on (that the solid would show "holes") was unfounded: the solid writes its depth in the opaque pass *before* any ghost pixel draws, so a non-depth-writing ghost can only blend a faint tint over it. Geometry was always correct (full-opacity renders pristine). The heavier candidates once on the table — global uniform opacity, dropping the tool, or order-independent transparency — proved unnecessary. **Explode + highlight** remain independent and unaffected.

**Verification.** Core **120** tests (+3 `jointFaces`), web **70** (+5 `explode`) — all green; all typecheck; web prod build clean, main bundle still 0 manifold / 0 BufferGeometry. Headless (swiftshader, `puppeteer-core`): a single M&T and a 3-board shared-member fixture render a **clean tenon-out-of-mortise reveal** under **explode** (both faces highlighted, connectivity case proven), and **full-opacity** isolate baselines are pristine. **0 JS/worker/WASM errors** (only the benign favicon 404). The isolate ghost depth-sort bug (above) is now fixed via `depthWrite:false` — re-verify the ghosted-M&T case visually. Screenshots in gitignored `docs/screenshots/chunk9/`.

**Deferred (fast-follow):** section/cutaway clipping plane; face-pick → joint selection (chunk 11 — this stage only *displays* provenance); persisting view toggles.
